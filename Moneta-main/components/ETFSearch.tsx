    ...
    <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto border border-white/20 animate-in zoom-in duration-300">
      <div className="sticky top-0 bg-white/80 backdrop-blur-md px-10 py-6 border-b border-slate-100 flex items-center justify-between z-10">
        <div>
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-emerald-600">Einfach & Neutral erklärt</p>
        </div>
      ...
      <div className="p-10 space-y-10">
        <div className="bg-emerald-50 p-6 rounded-3xl border border-emerald-100">
          ...
          <p className="text-sm font-medium text-slate-900 leading-relaxed">{comparison.comparison_summary}</p>
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm relative overflow-hidden">
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em]">Markt-Scanner</span>
          </div>
        ...
        <div className="flex gap-3 w-full md:w-auto relative z-10">
          {selectedETFs.length >= 2 && (
            <button 
              ...
              className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-600/20"
            >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {MOCK_ETFS.map(etf => (
                <tr key={etf.id} className={`group hover:bg-slate-50 transition-colors cursor-pointer ${selectedETFs.includes(etf.isin) ? 'bg-emerald-50/50' : ''}`} onClick={() => toggleSelection(etf.isin)}>
                  <td className="px-8 py-5">
                    <div className={`w-5 h-5 rounded-md border-2 transition-all flex items-center justify-center ${selectedETFs.includes(etf.isin) ? 'bg-emerald-600 border-emerald-600' : 'border-slate-200 group-hover:border-emerald-400'}`}>
                  ...
                  <td className="px-8 py-5">
                    <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-1 rounded-lg uppercase tracking-widest">{etf.category}</span>
                  </td>
