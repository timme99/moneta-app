    ...
    <div className="space-y-8 md:space-y-12 animate-in fade-in duration-700">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      <div className="h-full bg-emerald-600" style={{ width: `${holding.weight}%` }} />
                    </div>
      ...
      <div id="depot-news" className="bg-white rounded-[32px] md:rounded-[40px] border border-slate-200 p-6 md:p-8 shadow-sm">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
          ...
          <div className="bg-emerald-50 px-4 py-1.5 rounded-full border border-emerald-100 flex items-center gap-2 self-stretch sm:self-auto justify-center">
            <Sparkles className="w-3 h-3 text-emerald-600" />
            <span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">Relevanz-Filter Aktiv</span>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {report.news?.length > 0 ? report.news.map((item, i) => (
            <div key={i} className="p-6 bg-slate-50 rounded-[28px] md:rounded-[32px] border border-slate-100 flex flex-col group hover:border-emerald-200 transition-all hover:shadow-lg">
              ...
              <h4 className="font-bold text-slate-900 mb-2 group-hover:text-emerald-600 transition-colors leading-snug">
                {item.title}
              </h4>
              ...
              <button
                ...
                className="w-full py-3.5 md:py-4 bg-white border border-slate-200 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-600 hover:bg-emerald-600 hover:text-white transition-all flex items-center justify-center gap-2 shadow-sm"
              >
      ...
      {/* 4. NÄCHSTE SCHRITTE */}
      <div className="bg-emerald-600 rounded-[32px] md:rounded-[40px] p-8 md:p-12 text-white relative overflow-hidden shadow-2xl shadow-emerald-600/20">
            ...
            {report.nextSteps?.map((step, i) => (
              <div key={i} className="bg-white/10 backdrop-blur-md border border-white/20 p-6 rounded-[24px] md:rounded-[32px] flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-emerald-100 font-medium leading-relaxed">{step.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="fixed inset-0 z-[400] bg-slate-900/60 backdrop-blur-xl flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white rounded-t-[32px] sm:rounded-[40px] shadow-2xl w-full max-w-2xl overflow-hidden border border-white/20 animate-in slide-in-from-bottom sm:zoom-in duration-300">
            ...
            <div className="p-6 md:p-8 space-y-6 md:space-y-8 overflow-y-auto max-h-[70vh]">
              ...
              <div className="bg-emerald-600 p-6 md:p-8 rounded-[24px] md:rounded-[32px] text-white">
                <p className="text-sm font-medium leading-relaxed italic">"{newsImpact.educational_note}"</p>
              </div>
            </div>
            <div className="p-6 bg-slate-50 text-center border-t border-slate-100">
              <button onClick={() => setNewsImpact(null)} className="w-full sm:w-auto px-12 py-4 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-emerald-600 transition-all shadow-lg">Verstanden</button>
            </div>
          </div>
        </div>
      )}
